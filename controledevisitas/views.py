...
class ControleVisitaViewSet(ModuloRequeridoMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self, slug=None):
        ...
        proxima_visita = self.request.query_params.get('proxima_visita')
        ...
        if proxima_visita and proxima_visita.lower() == 'true':
            hoje = date.today()
            queryset = queryset.filter(ctrl_prox_visi__gte=hoje)
