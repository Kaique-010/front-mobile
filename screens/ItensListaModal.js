  ...
  const salvarItens = async () => {
    ...
    try {
      const payload = selecionados.map((produto) => {
        const item = {
          item_empr: Number(empresaId),
          item_fili: Number(filialId),
          item_list: Number(listaId),
          item_prod: Number(produto.prod_codi),
          item_fina: false,
          item_clie: Number(clienteId),
          item_pedi: 0,
          item_usua: Number(usuarioId),
        }
      ...
      // Chamada à API
      const response = await apiPostComContexto('itens/', payload)
      ...
      Alert.alert(
        ...
        `Falha ao salvar os itens. Detalhes: ${
          err.response?.data?.detail || err.response
        }`
      )
    }
  }
