...
const PainelAcompanhamento = ({ navigation }) => {
  ...
  const calcularContadores = (osnsData) => {
    const hoje = new Date()

    const abertas = osnsData.filter((o) => o.os_stat_os === 0).length
    const concluidas = osnsData.filter((o) => o.os_stat_os === 4).length
    const atrasadas = osnsData.filter((o) => o.os_stat_os === 2).length

    return { abertas, atrasadas, concluidas, total: osnsData.length }
  }

  const fetchosns = async () => {
    setLoading(true)
    try {
      const params = {}
      ...
      const response = await apiGetComContextoos('osmdeservico/osns/', {
        params,
      })

      const osnsData = Array.isArray(response)
        ? response
        : response.results || []

      setosns(osnsData)
      setContadores(calcularContadores(osnsData))
    } catch (error) {
      console.error('Erro ao buscar osns:', error.message)
      setosns([])
    } finally {
      setLoading(false)
    }
    ...
    <TouchableOpacity
      style={[
        styles.card,
        {
          ...
          bosrLeftColor: prioridadeColors[item.os_prio] || '#aaa',
        },
      ]}
      activeOpacity={0.7}
      onPress={() => navigation.navigate('osDetalhe', { osm: item })}>
      <View style={styles.cardHeader}>
        ...
        <View
          style={[
            ...
            { backgroundColor: prioridadeColors[item.os_prio] },
          ]}>
  ...
  return (
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          data={osns}
          keyExtractor={(item) => item.os_os.toString()}
