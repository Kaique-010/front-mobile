        ...
        <View style={{ backgroundColor: '#2f3e52', borderRadius: 8 }}>
          <Picker
            ...
            {filiais.map((filial, index) => {
              // Usar a mesma lógica da função buscarFiliais
              const filialId = 
                filial.fili_codi ||     // ✅ Propriedade correta (prioridade 1)
                filial.empr_empr ||     // ✅ Espelho da empresa (prioridade 2) 
                filial.filial_id ||     // ✅ Alternativa (prioridade 3)
                filial.id               // ✅ Fallback final
              
              return (
                <Picker.Item
                  key={filialId || `filial-${index}`}
                  label={filial.filial_nome || filial.empr_nome}
                  value={filialId?.toString() || '1'}
                />
              )
            })}
          </Picker>
