...
export default function CriarOrdemServico() {
  ...
  const [ordemServico, setOrdemServico] = useState({
    os_clie: null,
    os_clie_nome: '',
    os_data_abertura: new Date().toISOString(),
    pecas: [],
    servicos: [],
  })
  ...
  const validarOrdemServico = () => {
    ...
    if (!ordemServico.os_data_abertura) {
      Toast.show({
        type: 'error',
        text1: 'Data inválida',
            ...
            <>
              ...
              <TouchableOpacity
                ...
                <Text style={{ color: '#fff' }}>
                  {new Date(ordemServico.os_data_abertura).toLocaleDateString()}
                </Text>
              </TouchableOpacity>

              {showDatePicker && (
                <DateTimePicker
                  value={new Date(ordemServico.os_data_abertura)}
                  mode="date"
                  ...
                  onChange={(event, selectedDate) => {
                    ...
                    if (selectedDate) {
                      setOrdemServico((prev) => ({
                        ...prev,
                        os_data_abertura: selectedDate.toISOString(),
                      }))
                    }
                  }}
                />
