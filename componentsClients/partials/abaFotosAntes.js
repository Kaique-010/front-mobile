...
import { fetchClienteImagensAntes } from '../../services/clienteService'
...
const AbaFotosAntes = ({ ordemId }) => {
  ...
  const carregarImagens = async () => {
    try {
      setLoading(true)
      const data = await fetchClienteImagensAntes(ordemId)
      setImagens(data)
    } catch (error) {
      console.error('Erro ao carregar imagens antes:', error)
    } finally {
      setLoading(false)
    }
  }
